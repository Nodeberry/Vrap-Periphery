...
import "./SafeMath.sol";
import "./VrapLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library VrapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = VrapLibrary.getReserves(factory, tokenA, tokenB);

        require(
            reserveA > 0 && reserveB > 0,
            "VrapArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut =
                VrapLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut =
                VrapLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        IUniswapV2Pair pair =
            IUniswapV2Pair(VrapLibrary.pairFor(factory, tokenA, tokenB));
